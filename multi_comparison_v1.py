...
def og_tfx(name):
    tfx_results = []
    with open(f'top_{name}_probes_probe_beta_pairs.csv', 'r') as file:
    ...
    for c in out:
        ...
        tfx_results.append(np.median(tfx_list))
    return tfx_results
...
for name in range(1,11,2):
    ...
    preds = [x[1] for x in model.predict_proba([samples[i] for i in samples])]
    ...
    for i, j in zip(labels[1:], preds):
        print(f"logistic regression -- sample: {i} tumor fraction: {j}")
    for i, j in zip(labels[1:], results_tfx):
        print(f"og tfx -- sample: {i} tumor fraction: {j}")
